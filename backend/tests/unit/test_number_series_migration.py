...
async def _unique_indexes(conn, table: str) -> set[str]:
    return {row[1] for row in await conn.execute(text(f"PRAGMA index_list({table})")) if row[2]}
...
async def test_migrations_build_the_schema_and_re_run_cleanly(tmp_path, force_sqlite_dialect):
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            await conn.execute(text("DROP INDEX IF EXISTS ix_library_folders_number"))
            await conn.execute(text("ALTER TABLE projects DROP COLUMN number"))
            await conn.execute(text("ALTER TABLE library_folders DROP COLUMN number"))
        ...
        async with engine.begin() as conn:
            ...
            # The order folder is where the number is born, one step before the
            # project, so an upgraded install needs this column just as much.
            assert "number" in await _columns(conn, "library_folders")
            assert await _unique_indexes(conn, "library_folders") >= {"ix_library_folders_number"}
...
@pytest.mark.asyncio
async def test_a_folder_number_is_unique_but_missing_ones_are_not(tmp_path, force_sqlite_dialect):
    """Same shape as the project column one step earlier: every folder from
    before the series was switched on is unnumbered, and none of them may
    collide — but two folders may never share a number, because that is what an
    invoice is filed under."""
    engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'folders.db'}")
    try:
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
            await run_migrations(conn)
        ...
        async with sm() as db:
            db.add_all(
                [
                    LibraryFolder(name="Scratch"),
                    LibraryFolder(name="Older"),
                    LibraryFolder(name="", number="A-0007"),
                ]
            )
            await db.commit()

        async with sm() as db:
            db.add(LibraryFolder(name="Clash", number="A-0007"))
            with pytest.raises(Exception):  # noqa: B017 - IntegrityError, whatever the driver wraps it in
                await db.commit()
    finally:
        await engine.dispose()
