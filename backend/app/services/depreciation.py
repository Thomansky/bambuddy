...
A printer with ``wear_cost_per_hour`` set charges that amount for every hour
it runs. The value is a point-in-time snapshot taken at completion from the
printer row, so a later rate edit changes future runs only; nothing here ever
recalculates.
"""
...
def hourly_depreciation_rate(wear_cost_per_hour: float | None) -> float | None:
    """The wear rate per printing hour, or None when the feature is off.

    Unset or non-positive disables it: 0 is how the dialog says "no wear".
    """
    if not wear_cost_per_hour or wear_cost_per_hour <= 0:
        return None
    return wear_cost_per_hour


def run_depreciation_cost(duration_seconds: int | None, wear_cost_per_hour: float | None) -> float | None:
    ...
    """
    rate = hourly_depreciation_rate(wear_cost_per_hour)
...
async def snapshot_run_depreciation(
    ...
    if printer is None:
        return None
    cost = run_depreciation_cost(duration_seconds, printer.wear_cost_per_hour)
    if cost is None:
        return None
