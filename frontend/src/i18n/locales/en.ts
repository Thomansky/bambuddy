export default {
  ...
  printers: {
    ...
    drying: {
      ...
      waitingPaCalibration: 'Waiting for a flow-dynamics calibration to finish',
  ...
  archives: {
    ...
    filterUnconfirmedTitle: 'Show only prints still waiting for their outcome verdict',
    ...
    card: {
      ...
      printerWearDetail: '{{hours}} h × {{rate}} {{currency}}/h',
  ...
  stats: {
    ...
    suppliers: {
      title: 'By Supplier',
      empty: 'No purchases recorded yet. Mark on a spool which supplier it was bought from to group consumption and costs here.',
      supplier: 'Supplier',
      spools: 'Spools',
      remaining: 'Remaining',
      consumed: 'Consumed',
    ...
    rejectedPrintsCount: '{{rejected}} rejected after completion',
  ...
  maintenance: {
    ...
    section: {
      ...
      automaticHint: 'Bambuddy starts these on the printer when it is due and free — you only get told.',
      manual: 'You do it yourself',
      manualHint: 'A reminder. Bambuddy tracks when it is due; the work and the tick-off are yours.',
    },
    noItems: 'No maintenance set up yet',
    ...
    calibration: {
      option: {
        ...
        motor_noise: 'Motor noise cancellation',
        nozzle_offset: 'Nozzle offset',
        high_temp_heatbed: 'High-temperature bed leveling',
      ...
      runningSince: 'Calibration running since {{time}}',
      ...
      lastRunCancelled: 'Last run cancelled {{time}}',
      runQueuedToast: 'Calibration queued',
      runCancelledToast: 'Calibration run cancelled',
      runsCalibration: 'Runs a calibration',
    },
  },
  ...
  settings: {
    ...
    suppliers: {
      title: 'Suppliers',
      description: 'Manage where you buy filament — distinct from brand, which is who made it. Assign suppliers to spools in the spool dialog; a supplier that is still assigned to spools cannot be deleted.',
      search: 'Search suppliers...',
      addNew: 'Add new supplier',
      namePlaceholder: 'Name (e.g. Filament24)',
      websitePlaceholder: 'Website / shop URL',
      customerNumberPlaceholder: 'Customer no.',
      ...
      addFailed: 'Failed to add supplier',
      ...
      deleteBlocked: 'Cannot delete: still assigned to {{count}} spool(s)',
      loadFailed: 'Failed to load suppliers',
      deleteTitle: 'Delete Supplier',
      deleteConfirm: 'Delete "{{name}}"?',
      deleteConfirmReferenced: '"{{name}}" is assigned to {{count}} spool(s). Deleting is blocked until those assignments are removed.',
    },
    catalog: {
    ...
    libraryRootView: 'What the root shows',
    libraryRootViewAll: 'All files',
    libraryRootViewAllDescription: 'Every file in the library, in one flat list.',
    libraryRootViewFolders: 'Folders only',
    libraryRootViewFoldersDescription: 'The top-level folders, plus a "No folder" entry for the files outside any folder.',
    libraryRootViewRecent: 'Recently added or changed',
    libraryRootViewRecentDescription: 'The newest files across the whole library, up to 100 of them - a start page rather than a listing.',
    libraryRootViewDescription: 'Search and tag filters always look at the whole library, whichever of the three is picked.',
    webdavMode: 'WebDAV access',
    webdavDescription: 'Serve the File Manager tree over WebDAV so a workstation can map it as a drive.',
    webdavModeOff: 'Off',
    webdavModeOffDescription: 'The address answers 404, just like an install that never had the feature.',
    webdavModeRead: 'Read-only',
    webdavModeReadDescription: 'Files can be opened and copied; nothing done on the mapped drive changes the library.',
    webdavModeReadWrite: 'Read and write',
    webdavModeReadWriteDescription: 'Saving, renaming, moving and deleting from the drive as well, under the same library permissions the web interface applies.',
    ...
    webdavHttpsHint: 'Serve it over HTTPS, because the Basic credentials travel with every single request.',
    ...
    webdavWriteHint: 'Saving over a file keeps the same library entry, with its tags and notes, and replaces the content — no version and no variant. Deleting moves the file to the trash, and a folder created from the drive gets no number.',
    libraryStorageMode: 'Library storage',
    libraryStorageDescription: 'Where the File Manager keeps its files: in Bambuddy\'s own store, or in a directory tree it only indexes.',
    libraryStorageModeManaged: 'Bambuddy\'s own library',
    libraryStorageModeManagedDescription: 'Files are stored inside the Bambuddy data directory and are only reachable through Bambuddy. Nothing outside it can move them, so the database and the disk always agree.',
    libraryStorageModeDirectory: 'Directory',
    libraryStorageModeDirectoryDescription: 'A directory you choose — typically a network share — becomes the library itself: every folder is a real directory and every file keeps its real name, so the same tree can be opened in Explorer.',
    libraryStoragePath: 'Path to the directory',
    libraryStoragePathHint: 'Absolute path as the Bambuddy server sees it, so a share has to be mounted there first. It must exist, be writable, and lie outside Bambuddy\'s own data directories.',
    libraryStorageDriftHint: 'Anything moved or deleted directly in the directory is invisible to Bambuddy until the folder is scanned, so Scan becomes part of the routine rather than a one-off.',
    libraryStorageHashHint: 'Files found by a scan are not hashed — reading every file on a share costs a full pass over it — so they do not take part in duplicate detection. Files written through Bambuddy are hashed as before.',
    libraryStorageMigration: 'Move the existing library into the directory',
    libraryStorageMigrationHint: 'Switching the mode moves nothing by itself. This copies every file to its place in the tree, checks the copy, and only then removes the original. It can be run again at any time and will move whatever is left.',
    libraryStoragePlanButton: 'Show what would move',
    libraryStorageMigrateButton: 'Move now',
    libraryStoragePlanSummary: '{{files}} file(s), {{folders}} folder(s), {{size}} to move.',
    libraryStorageMigrationDone: '{{files}} file(s) moved, {{folders}} directory/ies created.',
    libraryStorageMigrationFailed: 'The migration could not be started.',
    libraryStoragePathRequired: 'Enter the path first.',
    ...
    libraryStorageScanDone: 'Scan finished: {{added}} added, {{removed}} removed.',
    libraryStoragePlanFirst: 'Show what would move first.',
    libraryStorageBlocked: 'The move is blocked by {{blockers}} name collision(s):',
    libraryStorageBlockerCollision: '{{first}} and {{second}} would both become the same file. Rename one of them.',
    libraryStorageBlockerExists: '{{name}} already exists there. Move or rename it on the share.',
    storageUsageLibraryTree: 'Library files are on {{path}}.',
    storageUsageLibraryTreeFree: '{{free}} free of {{total}}',
    // Updates
  ...
  fileManager: {
    ...
    renderingPreviews: 'Rendering previews… {{done}}/{{total}}',
    generateThumbnailsForMissing: 'Generate thumbnails for STL and PDF files without a preview',
    gridView: 'Grid view',
    listView: 'List view',
    ...
    noFolder: 'No folder',
    recentCrumb: 'Recent',
    sortedByRecent: 'Sorted by when files were added or changed',
    pathBar: {
      label: 'Location',
      showHidden: 'Show hidden folders',
      editPath: 'Edit path',
      noSuchFolder: 'No such folder',
      siblings: 'Folders at this level',
    },
    search: {
    ...
    preview: {
      ...
      msg: {
        ...
        rtfNote: 'Text extracted from rich-text content — formatting simplified.',
      },
    },
    details: {
      title: 'File details',
    ...
    toast: {
      ...
      previewsRendered: 'Rendered {{count}} preview(s) in the browser',
  ...
  inventory: {
    ...
    suppliers: {
      ...
      noneLeft: 'All suppliers are already assigned',
      createNew: 'Create "{{name}}"',
      createFailed: 'Failed to create supplier',
      purchaseSource: 'Bought here',
      purchaseSourceHelp: 'Marks where this spool was actually purchased; the other entries are alternative sources.',
      articleNumberPlaceholder: 'Supplier article no.',
      pricePlaceholder: 'Quoted price/kg',
      priceHelp: 'Quoted price at this supplier, for comparing sources — the spool\'s own cost per kg stays the cost basis.',
      help: 'Where this product can be bought. The highlighted entry is where this spool was purchased.',
      saveFailed: 'Failed to save supplier assignments',
    },
  ...
  confirmOutcome: {
    title: 'How did your print come out?',
    good: 'Good',
    reject: 'Reject',
    later: 'Ask me later',
    noPhoto: 'No finish photo available',
    savedGood: 'Marked as good part',
    savedReject: 'Marked as rejected',
  ...
  paCalibration: {
    ...
    menuAction: 'Flow dynamics calibration…',
    printer: 'Printer',
    nozzle: 'Nozzle',
    slot: 'Slot',
    currentK: 'Current K',
    ...
    cost: 'Takes',
    costValue: 'about {{minutes}} min, about {{grams}} g',
    plateType: 'Build plate on the bed',
    plateTypeHint: 'The line is sliced for this plate, so it has to match what is really on the bed.',
    presetsHint: 'Sliced with these presets. Pick the matching filament preset for a third-party spool.',
    plateConfirm: 'The build plate is empty and the printed line may be removed afterwards.',
    start: 'Start calibration',
    save: 'Save to printer',
    discard: 'Discard',
    cancelPrintConfirm: 'Stop the calibration print?',
    oldK: 'Stored K',
    newK: 'Measured K',
    nCoef: 'Flow coefficient',
    confidence: 'Confidence',
    willOverwrite: 'Saving replaces the existing K profile.',
    willCreate: 'Saving creates a new K profile and binds it to this slot.',
    notLoaded: 'Not loaded in any slot',
    noPermission: 'You do not have permission to start a print on this printer.',
    blocked: {
      model_not_supported: 'Not supported on this printer yet.',
      ...
      slicer_not_configured: 'The slicer sidecar is not reachable. This calibration is a sliced print, so it needs one.',
      slot_empty: 'The slot holds no identified filament.',
      nozzle_unknown: 'The printer has not reported which nozzle is fitted.',
      printer_preset_unavailable: 'The slicer has no bundled profile for this printer and nozzle.',
    },
    status: {
    ...
    waiting: {
      printer_offline: 'Waiting for the printer to come back online',
      printer_busy: 'Waiting for the printer to finish its current job',
      printer_reserved: 'Waiting for the queue to release the printer',
      plate_not_cleared: 'Waiting for the build plate to be acknowledged as clear',
    },
    plates: {
      cool_plate: 'Cool Plate',
      eng_plate: 'Engineering Plate',
      hot_plate: 'Smooth PEI Plate',
  ...
  notifications: {
    ...
    printConfirmRequest: 'Outcome Confirmation',
    printConfirmRequestDescription: 'Ask for a good/reject verdict with one-tap links when an opted-in print completes',
    ...
    paCalibration: 'Calibration',
    paCalibrationTitle: 'Flow Dynamics Calibration',
    paCalibrationDescription: 'A pressure-advance measurement finished and is waiting to be saved, or the run ended',
