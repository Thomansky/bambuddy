...
// from brand (who made it). Lives next to the spool catalog in the filament
// settings; spools reference suppliers from the spool dialog's multi-select.
export function SupplierSettings() {
  ...
  const loadSuppliers = useCallback(async () => {
    try {
      ...
      console.error('SupplierSettings.loadSuppliers failed:', err);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      console.error('SupplierSettings.handleAdd failed:', err);
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      ...
      console.error('SupplierSettings.handleUpdate failed:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error('SupplierSettings.handleDelete failed:', err);
  ...
  return (
    <Card id="card-suppliers">
      <CardHeader>
        <div className="flex items-center gap-2 mb-3">
          <Store className="w-5 h-5 text-bambu-gray" />
          <h2 className="text-lg font-semibold text-white">{t('settings.suppliers.title')}</h2>
          <span className="text-sm text-bambu-gray">({suppliers.length})</span>
          <button
            onClick={() => { setShowAddForm(true); setEditingId(null); resetForm(); }}
            className="ml-auto px-3 py-1.5 text-sm bg-bambu-green text-white rounded-lg hover:bg-bambu-green/80 transition-colors flex items-center gap-1.5"
          >
