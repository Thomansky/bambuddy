      ...
      <Card className="relative">
        <CardContent className="p-3">
          ...
          <div className="mb-3 flex flex-wrap gap-1">
            ...
            {provider.on_pa_calibration && (
              <span className="px-2 py-0.5 bg-teal-100 dark:bg-teal-600/20 text-teal-700 dark:text-teal-300 text-xs rounded">{t('notifications.paCalibration')}</span>
            )}
...
<Toggle
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-white">{t('notifications.paCalibrationTitle')}</p>
                    <p className="text-xs text-bambu-gray">{t('notifications.paCalibrationDescription')}</p>
                  </div>
                  <Toggle
                    checked={provider.on_pa_calibration ?? false}
                    onChange={(checked) => updateMutation.mutate({ on_pa_calibration: checked })}
                  />
                </div>
              </div>
